...
def aggregate(sdata_obj,ref_shape_name : str, ref_table_name : str, coord_system, ref_region_colname = "region",
    ...
    old_region_name = ref_table_shape.obs.region.unique()[0]
    ...
    values_['instance_areas'] = values_.geometry.area
    ...
    intersect_df = pd.merge(intersect_df, values_[['instance','instance_areas']], how = 'left')
    intersect_df['cell_fraction'] = intersect_df.intersect_areas / intersect_df.cell_areas

    return intersect_df


pixel_intersect_df = aggregate(sdata_obj = sdata, ref_shape_name = "ion_image_polyg", ref_table_name = "ion_table",
                               ref_region_colname = "region", coord_system = "aligned", actual_val_key = "instance")
pixel_intersect_df.to_csv(f"{CLUSTER_BASE_DIR}/Results/Upstream/SpatialData/updated_geopandas_intersect_cells_pixels.csv")